...
export function WarehouseKirimWizard({ warehouseId, onClose, onSaved }: Props) {
  ...
  const updM = (id: string, f: keyof KirimMeasurement, v: string) =>
    upd({ measurements: cur.measurements.map(m => m.id === id ? { ...m, [f]: v } : m) });
  const addM = () => upd({ measurements: [...cur.measurements, { id: crypto.randomUUID(), value: "", unit: "kg" }] });
  const delM = (id: string) => {
    if (cur.measurements.length <= 1) return;
    upd({ measurements: cur.measurements.filter(m => m.id !== id) });
  };

  const updP = (id: string, f: keyof KirimPlace, v: string) =>
    upd({ places: cur.places.map(p => p.id === id ? { ...p, [f]: v } : p) });
  const addP = () => upd({ places: [...cur.places, { id: crypto.randomUUID(), count: "", unit: "joy" }] });
