...
function EmployeeArchive() {
  ...
  const handleRowClick = async (log: ActivityLog) => {
    if (getCategory(log.actionType) === "TASK" && log.details?.taskId) {
      setViewTaskId(log.details.taskId);
      try {
        ...
        toast.error("Vazifani yuklashda xatolik: " + err.message);
        setViewTaskId(null);
      }
    }
  };
            ...
            <div className="flex flex-col">
              <div className="p-8 border-b border-border bg-secondary/10 relative">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="p-5 rounded-2xl bg-background/50 border border-border font-medium text-foreground whitespace-pre-wrap leading-relaxed shadow-inner">
                      {viewTask.description || "Ushbu topshiriq uchun qo'shimcha tavsif berilmagan."}
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-8 p-6 rounded-[32px] border border-border/50 bg-secondary/5">
                  ...
                  <TaskHistory templateId={viewTask.templateId} currentInstanceId={viewTask.id} />
                </div>
              </div>

              <div className="p-8 flex gap-4 justify-end">
                <button
                  onClick={() => { setViewTaskId(null); setViewTask(null); }}
